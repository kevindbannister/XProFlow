...
export const DashboardView: FC<DashboardViewProps> = ({ visibility }) => {
  const maxTrendValue = Math.max(...automationTrend.map((point) => point.value));

  return (
    <div className="space-y-10 text-slate-800">
      {visibility['overview.hero'] ? (
      ...
      <div className="grid gap-8 lg:grid-cols-[2fr,1fr]">
        <div className="space-y-8">
          {(visibility['overview.activity'] || visibility['overview.categoryBreakdown']) && (
            <section className="grid gap-6 lg:grid-cols-2">
              {visibility['overview.activity'] ? (
              ...
              {visibility['overview.categoryBreakdown'] ? (
                <div className="rounded-[32px] border border-white/70 bg-white/90 p-6 shadow-[0_30px_60px_rgba(15,23,42,0.1)]">
          ...
          {visibility['overview.weeklySummary'] ? (
        ...
        {visibility['overview.quickActions'] ? (
