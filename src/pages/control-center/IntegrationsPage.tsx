      ...
      {integrations.map((integration) => (
        <SettingsCard key={integration.name} className="space-y-4">
          <div className="flex items-center gap-3">
            <span className="flex h-10 w-10 items-center justify-center rounded-full bg-slate-100 text-lg dark:bg-slate-800">{integration.logo}</span>
            <div>
              <h2 className="text-base font-semibold text-slate-900 dark:text-slate-100">{integration.name}</h2>
              <p className="text-sm text-slate-500 dark:text-slate-300">Connection status: {integration.status}</p>
            </div>
          </div>
