...
const Sidebar = () => {
  const renderItem = (label: string, to: string, Icon: typeof LayoutDashboard) => (
    ...
    >
      ...
      <span className="dropdown-surface pointer-events-none absolute left-8 top-1/2 hidden -translate-y-1/2 rounded-md border px-2 py-1 text-xs font-medium theme-text-secondary shadow-sm group-hover:block">
        {label}
      </span>
    </NavLink>
  );
  ...
  return (
    <aside className="sidebar-surface fixed bottom-0 left-0 top-10 z-40 flex w-12 flex-col items-center justify-between border-r py-2">
