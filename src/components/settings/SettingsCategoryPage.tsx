...
const SettingsCategoryPage = ({ title, subtitle, description, sections }: SettingsCategoryPageProps) => (
  <main className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-950 lg:p-5" aria-label={`${title} settings page`}>
    <section className="min-h-[32rem] rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900 lg:p-6">
      ...
      <div className="grid gap-3 lg:grid-cols-2">
        {sections.map((section) => (
          <article key={section.heading} className="rounded-lg border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-950">
