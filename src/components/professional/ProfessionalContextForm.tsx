...
const ProfessionalContextForm = ({ mode }: { mode: 'onboarding' | 'settings' }) => {
  ...
  return (
    <section className="flex w-full max-w-6xl flex-col gap-5">
      <PageHeader
        ...
        action={(
          <div className="flex flex-wrap gap-2">
            <FilterPill active={mode === 'onboarding'}>{mode === 'onboarding' ? 'Onboarding' : 'Settings'}</FilterPill>
            <FilterPill>Live profile</FilterPill>
          </div>
        )}
      />
      ...
      <Card className="space-y-5">
        <div>
          <p className="text-sm font-medium text-content-primary">Specialisms</p>
          <div className="mt-2 flex flex-wrap gap-2">
          {SPECIALISMS.map((item) => (
              <FilterPill key={item} active={context.user.specialisms.includes(item)} onClick={() => setContext({ ...context, user: { ...context.user, specialisms: toggleArrayValue(context.user.specialisms, item) } })}>
                {item}
              </FilterPill>
          ))}
          </div>
        ...
        <div>
          <p className="text-sm font-medium text-content-primary">Typical email audiences *</p>
          <div className="mt-2 flex flex-wrap gap-2">
          {AUDIENCES.map((item) => (
              <FilterPill key={item} active={context.user.audiences.includes(item)} onClick={() => setContext({ ...context, user: { ...context.user, audiences: toggleArrayValue(context.user.audiences, item) } })}>
                {item}
              </FilterPill>
          ))}
          </div>
        ...
        <div className="rounded-[12px] bg-surface-page p-4 shadow-card">
          <p className="text-xs font-semibold uppercase tracking-wide text-content-secondary">AI Context Block preview</p>
          <pre className="mt-2 whitespace-pre-wrap text-xs text-content-primary">{contextPreview}</pre>
        </div>
